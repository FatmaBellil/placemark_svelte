...
/* empty css                          */const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })} ${placemark ? `<table class="table is-fullwidth"><thead data-svelte-h="svelte-g0jn24"><tr><th>Name</th> <th>Description</th> <th>Latitude</th> <th>Longitude</th> <th>temperature</th> <th>Wether</th></tr></thead> <tbody><tr><td>${escape(placemark.name)}</td> <td>${escape(placemark.description)}</td> <td>${escape(placemark.latitude)}</td> <td>${escape(placemark.longitude)}</td> <td>${escape(temperature)}</td> <td>${escape(weatherMain)}</td></tr></tbody></table>` : `<p data-svelte-h="svelte-qdsr2u">Loading...</p>`}`;
});
export {
  Page as default
};
